...
function normalizeLocation(row) {
  const total = numberValue(row.total_pizzas ?? row.pizzas_analizadas ?? row.total);
  const pass = numberValue(row.pass_count ?? row.pass ?? row.pass_total);
  const fail = numberValue(row.fail_count ?? row.fail ?? row.fail_total);
  const averageScore = numberValue(row.average_score ?? row.avg_score ?? row.score);

  return {
    ...row,
    location: row.location ?? row.locacion ?? row.name ?? "Sin nombre",
    location_slug: row.location_slug ?? row.slug ?? "",
    relative_url: row.relative_url ?? "./",
    total_pizzas: total,
    pass_count: pass,
    fail_count: fail,
    average_score: averageScore,
    burbuja_count: numberValue(row.burbuja_count),
    grasa_count: numberValue(row.grasa_count),
    bordes_count: numberValue(row.bordes_count),
    best_item: row.best_item ?? null,
    worst_item: row.worst_item ?? null
  };
}
...
function normalizeSummary(summary, locations) {
  const totalLocations = locations.length;
  const totalPizzas = locations.reduce((acc, x) => acc + x.total_pizzas, 0);
  const passCount = locations.reduce((acc, x) => acc + x.pass_count, 0);
  const failCount = locations.reduce((acc, x) => acc + x.fail_count, 0);

  const weightedScore = totalPizzas
    ? locations.reduce((acc, x) => acc + x.average_score * x.total_pizzas, 0) / totalPizzas
    : 0;
  ...
  return {
    ...
    fail_count:
      ...
      failCount,
    company:
      summary.company ?? "PCSAPI",
    display_name:
      summary.display_name ??
      summary.district ??
      summary.district_slug ??
      "Distrito"
  };
}
...
async function main() {
  const [summaryPayload, locationPayload] = await Promise.all([
    fetchJson("./json/district_summary.json"),
    fetchJson("./json/district_locations.json")
  ]);

  let locations = normalizeLocationPayload(locationPayload)
    .slice()
    .sort((a, b) => locationScore(b) - locationScore(a));

  const summary = normalizeSummary(summaryPayload || {}, locations);
  ...
  console.log("[district] summary", summary);
  console.log("[district] locations", locations);
}
